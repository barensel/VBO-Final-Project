...
coins = ['BTC-USD', 'ETH-USD', 'BNB-USD', 'SOL-USD', 'DOGE-USD', 'AVAX-USD', 'MATIC-USD', 'ATOM-USD', 'DOT-USD']
...
def app():
    for i in coins:
        if i in coins_selected or select_all:
            ...
            tickerDf = yf.download([i], start=dt.date(2022, 1, 1), end=dt.date(2022, 3, 3))
            name = i.split("-")[0]
            ...
            tickerDf.reset_index()["Close"][len(tickerDf.reset_index()["Close"]) - 1] = df["Close"][0]

            fig = go.Figure()
            fig.add_trace(go.Scatter(x=tickerDf.reset_index()['Date'], y=tickerDf.reset_index()['Close'], name="History"))
            fig.add_trace(go.Scatter(x=df['Date'], y=df['Close'], name="Prediction"),)
...
def part2(avg_score, var,conditional_var, norm_var, norm_cvar, t_var, t_cvar,cc,lc,ch):
    ...
    st.markdown(
        f"<div>Our rate of return for this portfolio with Close-Close: <span class='highlight red'>{cc}</span></span></div>",
        unsafe_allow_html=True)
    st.markdown(" ")
    st.markdown(
        f"<div>Our rate of return for this portfolio with Close-High: <span class='highlight red'>{lc}</span></span></div>",
        unsafe_allow_html=True)
    st.markdown(" ")
    st.markdown(
        f"<div>Our rate of return for this portfolio with Low-High: <span class='highlight red'>{ch}</span></span></div>",
        unsafe_allow_html=True)
    st.markdown(" ")
    st.markdown(
