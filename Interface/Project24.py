...
def app():
    for i in coins:
        if i in coins_selected:
            tickerDf = yf.download([i], start=dt.date(2022, 1, 1), end=dt.date(2022, 6, 3))
            name = i.split("-")[0]
            ...
            df = pd.read_csv(rf"C:\Users\baren\PycharmProjects\pythonProject1\Forecast2\forecast_{name}_with_PyCaret_Regression.csv")
...
def part2(var,conditional_var, norm_var, norm_cvar, t_var, t_cvar, cc, ch, lh, avg_score_cc, avg_score_lc, avg_score_lh,genre):
    ...
    st.markdown(" ")
    st.markdown(
        f"<div>Our rate of return for this portfolio with Close-High: <span class='highlight red'>{ch}</span></span></div>",
        unsafe_allow_html=True)
    st.markdown(" ")
    st.markdown(
        f"<div>Our rate of return for this portfolio with Low-High: <span class='highlight red'>{lh}</span></span></div>",
        unsafe_allow_html=True)
    st.markdown(" ")
    st.markdown(
...
def portfolio(coins_selected):
    ...
    col4.metric("Avg Model Score", avg_score_cc)
